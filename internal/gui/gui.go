...
func confirmResetAllPreferences() bool {
	script := `tell application "System Events" to return button returned of (display dialog "Reset all goports preferences to defaults? This cannot be undone." buttons {"Cancel", "Reset All"} default button "Cancel" with icon caution)`
	out, err := exec.Command("osascript", "-e", script).Output()
	if err != nil {
		return false
	}
	return strings.TrimSpace(string(out)) == "Reset All"
}
...
func portTitle(key ports.PortKey, entries []ports.PortEntry) string {
	...
	if len(pidStrs) == 0 && len(entries) > 0 {
		title += " [no PID]"
	}
	return title
}
...
func onReady() {
	...
	nativeItem := settingsItem.AddSubMenuItemCheckbox("Use native discovery only", "Same as CLI --native; skips lsof (may hide PIDs — see README)", false)
	...
	resetAllItem := settingsItem.AddSubMenuItem("Reset all preferences…", "Restore all defaults (TCP/UDP, notifications, native mode, filter, webview, interval)")
	...
	go func() {
		for range resetAllItem.ClickedCh {
			if !confirmResetAllPreferences() {
				continue
			}
			if err := config.ResetToDefaults(); err != nil {
				logf("reset all preferences: %v\n", err)
				continue
			}
			cfg = config.Load()
			ports.SetNativeOnly(cfg.NativeOnly)
			...
			yItem.SetTitle(fmt.Sprintf("Webview Y (%d)", cfg.WebviewY))
			titleItem.SetTitle(fmt.Sprintf("Webview title (%s)", cfg.WebviewTitle))
			_ = beeep.Notify("goports", "All preferences reset to defaults", "")
		}
	}()
	...
	for {
		...
		for _, key := range keys {
			...
			if !exists {
				...
				if allPIDZero && runtime.GOOS == "darwin" {
					desc += ". PIDs hidden: try turning off ‘Use native discovery only’ in Settings, or grant Full Disk Access if macOS blocks process info."
				}
